...
static bool verticesEqual(Vec2 a, Vec2 b) {
    float dx = a.x - b.x, dy = a.y - b.y;
    return dx * dx + dy * dy < 1e-4f;
}

struct VHit {
    int sector = -1, index = -1;
};
    ...
    for(int s = 0; s < (int)m.sectors.size(); ++s) {
        ...
        for(int w = 0; w < n; ++w) {
            ...
            if(d < best) {
                best = d;
                h = {s, w};
                proj = {a.x + (b.x - a.x) * t, a.y + (b.y - a.y) * t};
            }
        }
    }
    return h;
...
static bool addSector(Map& m, std::vector<Vec2> pts) {
    ...
    Sector S;
    S.floor = 0.0f;
    S.ceiling = 4.5f; // neutral defaults
    S.floorColor = 0xFF243447;
    S.ceilingColor = 0xFF1b2230;
    S.wallColor = 0xFF6f7d8c;
    Vec2 c{0, 0};
    for(auto& v : pts) {
        c.x += v.x;
        c.y += v.y;
    ...
    for(const Sector& E : m.sectors) // inherit from the sector we're inside
        if(pointInSector(E, c)) {
            S.floor = E.floor;
            S.ceiling = E.ceiling;
            S.floorColor = E.floorColor;
            S.ceilingColor = E.ceilingColor;
            S.wallColor = E.wallColor;
            break;
        }
    S.vertices = pts;
...
int main(int argc, char** argv) {
    ...
    auto pushUndo = [&]() {
        undo.push_back({map.sectors, map.sprites});
    ...
    auto snap = [&](Vec2 p) {
        if(gridSnap && !(SDL_GetModState() & KMOD_ALT)) {
    ...
    // ---- texture picker (B) ----
    bool browsing = false;
    int browsePage = 0, browseHover = -1;
    SurfaceRef pickTarget;
    ...
    while(running) {
        ...
        while(SDL_PollEvent(&e)) {
            if(e.type == SDL_QUIT) running = false;
            ...
            } else if(e.type == SDL_MOUSEBUTTONDOWN && browsing &&
                      e.button.button == SDL_BUTTON_LEFT) {
                const int per = Renderer::BR_COLS * Renderer::BR_ROWS;
                const int cw = W / Renderer::BR_COLS,
                          ch = (H - Renderer::BR_HEAD) / Renderer::BR_ROWS;
                int mx = e.button.x, my = e.button.y;
                if(my >= Renderer::BR_HEAD) {
                    int col = mx / cw, row = (my - Renderer::BR_HEAD) / ch;
                    int vi = browsePage * per + row * Renderer::BR_COLS +
                             col; // index into the filtered view
                ...
                mvScale = clampf(mvScale * (e.wheel.y > 0   ? 1.15f
                                            : e.wheel.y < 0 ? 0.87f
                                                            : 1.0f),
                                 2.0f, 400.0f);
                mvOx = mouseX - wx * mvScale;
                mvOy = mouseY + wy * mvScale;
            } else if(e.type == SDL_MOUSEBUTTONDOWN && mapView &&
                      e.button.button == SDL_BUTTON_RIGHT) {
                if(drawing) {
                    drawing = false;
                    drawPts.clear();
                    printf("draw cancelled\n");
                }
            } else if(e.type == SDL_MOUSEBUTTONDOWN && mapView &&
                      e.button.button == SDL_BUTTON_LEFT) {
                mouseX = e.button.x;
                ...
                if(drawing) { // place / close a new-sector point
                    ...
                    if(drawPts.size() >= 3) { // click near the first point closes
                        float sx = mvOx + drawPts[0].x * mvScale,
                              sy = mvOy - drawPts[0].y * mvScale;
                        ...
                        if(dx * dx + dy * dy < 100.0f) {
                            ...
                            if(addSector(map, drawPts)) {
                                ...
                                undo.pop_back();
                                printf("invalid sector (need 3+ non-collinear points)\n");
                    ...
                    } else if((dragSprite = pickSprite(map, mvScale, mvOx, mvOy, mouseX, mouseY)) >=
                              0) {
                        pushUndo(); // grab a sprite to reposition
                    } else {        // else split the wall here
                        Vec2 proj;
                        VHit w = pickWall(map, mvScale, mvOx, mvOy, mouseX, mouseY, proj);
                        if(w.index >= 0) {
                            pushUndo();
                            proj = snap(proj);
                            splitWall(map, w.sector, w.index, proj);
                            rebuildPortals(map);
                            collectCoincident(map, proj, dragVerts);
                        }
                    }
                }
            } else if(e.type == SDL_MOUSEBUTTONUP && mapView &&
                      e.button.button == SDL_BUTTON_LEFT) {
                dragVerts.clear();
                dragSprite = -1;
            }
                ...
                if(browsing) { // ---- texture picker is modal ----
                    ...
                    if(k == SDLK_ESCAPE || k == SDLK_b) {
                        ...
                        browsePage = clampi(browsePage + 1, 0, pages - 1);
                    else if(k == SDLK_LEFT || k == SDLK_UP || k == SDLK_PAGEUP)
                        browsePage = clampi(browsePage - 1, 0, pages - 1);
                } else {
#endif
                    ...
                    if(k == SDLK_b && !mapView) { // open the picker on the aimed wall/sprite
                        ...
                        if(a.kind == SurfaceRef::None) showMessage("aim at a wall or sprite first");
                            ...
                            if(a.kind == SurfaceRef::Sprite) {
                                targetSprite = a.sprite;
                                pickTarget = SurfaceRef{};
                            } else {
                                pickTarget = a;
                                targetSprite = -1;
                            }
                    ...
                    if(k == SDLK_RETURN || k == SDLK_KP_ENTER) {
                        mapView = !mapView;
                        drawing = false;
                        drawPts.clear(); // leaving the view cancels a draw
                        if(mapView) fitView();
                    ...
                    if(mapView) { // ---- 2D map-editor keys ----
                        ...
                        if(k == SDLK_b) { // start / finish drawing a new sector
                            if(drawing) {
                                ...
                                drawing = true;
                                drawPts.clear();
                                printf("draw sector: click points; click the first point or press "
                                       "B to close, right-click cancels\n");
                        ...
                        if(drawing) {
                            ...
                            if(k == SDLK_DELETE || k == SDLK_BACKSPACE || k == SDLK_x) {
                                ...
                                if(v.index >= 0) { // delete a vertex...
                                    ...
                                    if(!deleteVertex(map,
                                        ...
                                        printf(
                                            "can't delete: a sector would drop below 3 vertices\n");
                            ...
                            if(k == SDLK_n) { // drop a new sprite at the cursor
                                ...
                                for(const Sector& S : map.sectors) // rest on that sector's floor
                                    if(pointInSector(S, wp)) {
                                        s.z = S.floor;
                                        break;
                                    }
                                map.sprites.push_back(s);
                                printf("added sprite %d\n", (int)map.sprites.size() - 1);
                                showMessage("added sprite " +
                                            std::to_string((int)map.sprites.size() - 1));
                            }
                        }
                    }
                    ...
                    if(k == SDLK_n && !mapView) { // cycle image texture on the aimed surface (3D)
                        ...
                        if(a.sector >= 0 && a.sector < (int)map.sectors.size()) {
                            ...
                            if(a.kind == SurfaceRef::Wall && a.wall < (int)sc.wallTextureIds.size())
                                idp = &sc.wallTextureIds[a.wall];
                    ...
                    if(k == SDLK_o && !mapView) { // toggle sky backdrop on the aimed ceiling (3D)
                        SurfaceRef a = renderer.pickAt(W / 2, H / 2);
                        if(a.kind == SurfaceRef::Ceiling && a.sector >= 0 &&
                           a.sector < (int)map.sectors.size()) {
...
#if EDITOR
        if(browsing) { /* texture picker is modal: freeze the world */
        } else if(mapView) {
...
#endif
        {
            ...
            if(editMode) {
                    ...
                    if(aim.kind == SurfaceRef::Sprite && aim.sprite >= 0 &&
                       aim.sprite < (int)map.sprites.size()) {
                        ...
                        for(const Sector& S : map.sectors) // floor, head under the ceiling
                            if(pointInSector(S, sp.position)) {
                                lo = S.floor;
                                hi = std::max(S.floor, S.ceiling - sp.height);
                                break;
                            }
                        ...
                        if(player.camera.pitch < 0.0f) { // looking up -> ceiling
                            ...
                            if(ks[SDL_SCANCODE_G])
                                t.ceiling = std::max(t.ceiling - rate, t.floor + 0.3f);
                        } else { // looking down / level -> floor
                            if(ks[SDL_SCANCODE_T])
                                t.floor = std::min(t.floor + rate, t.ceiling - 0.3f);
                            if(ks[SDL_SCANCODE_G]) t.floor = std::max(t.floor - rate, -8.0f);
                        }
                    }
                }
                // Y / H brighten / darken the exact surface under the crosshair
                if((ks[SDL_SCANCODE_Y] || ks[SDL_SCANCODE_H]) && aim.sector >= 0 &&
                   aim.sector < (int)map.sectors.size()) {
                    Sector& s = map.sectors[aim.sector];
                    float* L = nullptr;
                    if(aim.kind == SurfaceRef::Wall && aim.wall < (int)s.wallLight.size())
                        L = &s.wallLight[aim.wall];
                ...
                if(aim.sector >= 0 && aim.sector < (int)map.sectors.size()) {
                    ...
                    if(aim.kind == SurfaceRef::Wall && aim.wall < (int)t.wallTextures.size())
                        tx = &t.wallTextures[aim.wall];
        ...
        if(mapView) {
            ...
            if(!drawing) { // no hover highlight while drawing
                ...
                hovSprite =
                    dragSprite >= 0
                        ? dragSprite
                        : (hv.index < 0 ? pickSprite(map, mvScale, mvOx, mvOy, mouseX, mouseY)
                                        : -1);
            }
...
#endif
        {
            ...
            renderer.drawMinimap(map, player.camera, editMode ? aim : SurfaceRef{}, map.playerStart,
                                 map.startAngle);
        ...
        if(browsing) { // texture picker over the frozen scene
            ...
            renderer.drawTextureBrowser(browseTex, browseView, browsePage, browseHover,
                                        filterName());
        }
#endif
        ...
        for(int i = 0; i < (int)toasts.size(); ++i) {        // transient messages, newest at bottom
            ...
            renderer.drawText(x + 1, y + 1, toasts[i].text.c_str(), (uint32_t)alpha << 24,
                              scale); // shadow
            renderer.drawText(x, y, toasts[i].text.c_str(), ((uint32_t)alpha << 24) | 0xe0f0ff,
                              scale);
        }
#endif

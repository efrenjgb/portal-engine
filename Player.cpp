...
void Player::move(const Map& map, float dx, float dy){
    for(int iter = 0; iter < 4; ++iter){            // slide, then re-check
        ...
        for(int s = 0; s < n; ++s){
            ...
            if(segCross(camera.x, camera.y, camera.x+dx, camera.y+dy, a.x, a.y, b.x, b.y)){ hit = s; break; }
        }
        if(hit < 0) break;
...
void Player::keepInside(const Map& map){
    ...
    for(int s = 0; s < n; ++s){
        ...
        if(d < margin){
            if(d > 1e-6f){ camera.x += dx/d * (margin - d); camera.y += dy/d * (margin - d); }
            else { float L = std::sqrt(L2); camera.x += -ey/L*margin; camera.y += ex/L*margin; }
        }
    }
}
...
void Player::collideSprites(const Map& map){
    ...
    for(const Sprite& s : map.sprites){
        ...
        if(d2 >= r*r) continue;
        float d = std::sqrt(d2);
        if(d > 1e-6f){ camera.x += dx/d * (r - d); camera.y += dy/d * (r - d); }
        else         { camera.x += r; }
    }
}
...
void Player::jump(){
    if(onGround){ zVelocity = JUMP_VELOCITY; onGround = false; }
}
...
void Player::settleEyeHeight(const Map& map, float dt){
    ...
    if(onGround && zVelocity == 0.0f){
        ...
        if(camera.z <= groundEye){ camera.z = groundEye; zVelocity = 0.0f; onGround = true; }
        else onGround = false;
    }
}
